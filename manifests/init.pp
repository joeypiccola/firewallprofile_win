...
class firewallprofile_win (
  ...
  registry_value { 'HKLM\CurrentControl\SetServices\SharedAccess\Parameters\FirewallPolicy\StandardProfile\EnableFirewall':
    ensure => present,
    type   => dword,
    data   => $standard_profile_value,
  }
  
  registry_value { 'HKLM\CurrentControl\SetServices\SharedAccess\Parameters\FirewallPolicy\DomainProfile\EnableFirewall':
    ensure => present,
    type   => dword,
    data   => $domain_profile_value,
  }

  registry_value { 'HKLM\CurrentControl\SetServices\SharedAccess\Parameters\FirewallPolicy\PublicProfile\EnableFirewall':
    ensure => present,
    type   => dword,
